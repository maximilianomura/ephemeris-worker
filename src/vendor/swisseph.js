...
import WasmSwissEph from './swisseph-factory.js';
...
class SwissEph {
  ...
  // wasmModule: pre-compiled WebAssembly.Module (required in CF Workers to bypass
  // relative-URL fetch limitations). Provided via: import wasm from 'path/to/swisseph.wasm'
  async initSwissEph(wasmModule) {
    let moduleConfig = {};

    // Detect Cloudflare Workers: WorkerGlobalScope exists but window does not.
    // Must check this BEFORE the nodejs_compat polyfill makes process.versions.node truthy.
    const isCFWorker = typeof globalThis.WorkerGlobalScope !== 'undefined' &&
      typeof globalThis.window === 'undefined';
    
    if (isCFWorker) {
      // Skip the 11.8 MB .data download — safe because we use SEFLG_MOSEPH (Moshier
      // built-in ephemeris) which embeds all data inside the WASM binary itself.
      moduleConfig.getPreloadedPackage = (_name, _size) => new ArrayBuffer(0);

      // Provide the pre-compiled WebAssembly.Module injected by Wrangler's module
      // binding system. This bypasses `new URL('swisseph.wasm', import.meta.url)` and
      // relative-URL fetch, both of which fail inside bundled CF Workers.
      if (wasmModule) {
        moduleConfig.instantiateWasm = (imports, callback) => {
      ...
      // Real Node.js environment, we need to help locate the WASM and data files
      try {
      ...
      // Browser environment (not CF Workers, not Node.js)
      moduleConfig.locateFile = (path, prefix) => {
        if (path.endsWith('.data') || path.endsWith('.wasm')) {
